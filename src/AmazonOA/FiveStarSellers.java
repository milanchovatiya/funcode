...
//    The array productRatings contains only non-negative integers.
    ...
    public int findMinRatingNeeded(int[][] ratings, int threshold) {
        PriorityQueue<Pair<Double, Integer>> maxHeap = new PriorityQueue<>((p1, p2) -> {
        ...
        double avg = 0.0;
        int minimumReviewsNeeded = 0;
        int totalProducts = ratings.length;
        for(int i = 0; i < ratings.length; i++) {
            int fiveStarReview = ratings[i][0];
            int totalReviews = ratings[i][1];
            if(fiveStarReview != totalReviews) {
                int incrementedFiveStarReview = ratings[i][0] + 1;
                int incrementedTotalReviews = ratings[i][1] + 1;
                double diff = (double) incrementedFiveStarReview / incrementedTotalReviews
                        - (double) fiveStarReview / totalReviews;
                maxHeap.add(new Pair<>(diff, i));
            }
            avg += (double)fiveStarReview/totalReviews;
        }
        avg /= totalProducts;

        while(maxHeap.size() > 0 && avg * 100 < threshold) {
            int maxIndex = maxHeap.remove().getValue();
            int fiveStarReview = ratings[maxIndex][0];
            int totalReviews = ratings[maxIndex][1];
            avg -= ((double)fiveStarReview/totalReviews) / totalProducts;
            ratings[maxIndex][0]++;
            ratings[maxIndex][1]++;
            avg += ((double)ratings[maxIndex][0]/ratings[maxIndex][1]) / totalProducts;
            int newFiveStarReview = ratings[maxIndex][0] + 1;
            int newTotalReviews = ratings[maxIndex][1] + 1;
            double newDiff = (double)newFiveStarReview/newTotalReviews - (double)ratings[maxIndex][0]/ratings[maxIndex][1];
            maxHeap.add(new Pair<>(newDiff, maxIndex));
            minimumReviewsNeeded++;
        }
        return minimumReviewsNeeded;

    }

    @Test
    public void findMinRatingNeededTest() {

        Assert.assertEquals(3, findMinRatingNeeded(new int[][]{
                {4, 4},
                {1, 2},
                {3, 6}
        }, 77));
